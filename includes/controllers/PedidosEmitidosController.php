    ...
    public static function getPedidosEmitidos() {
        ...
        if (!empty($emitidos)) {
            foreach ($emitidos as $pedido) {
                ...
                $html .= '<td class="text-right">' . esc_html($pedido['folios']) . '</td>';
                $html .= '<td>' . esc_html(date('d-m-Y', strtotime($pedido['document_date']))) . '</td>';
                $html .= '<td>' . esc_html($pedido['rut_receptor']) . '</td>';
                $html .= '<td class="text-right">' . esc_html(number_format($pedido['total'], 0, ',', '.')) . '</td>';
                $html .= '<td>';
                $statusClass = '';
                ...
                $html .= '</td>';
                $html .= '<td><button class="btn btn-info btn-sm view-dte-btn" data-order-id="' . esc_attr($pedido['order_id']) . '"><i class="fas fa-file-alt"></i> Ver DTE</button></td>';
                $html .= '</tr>';
            }
        } else {
